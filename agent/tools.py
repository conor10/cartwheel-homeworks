...
def search_products(
    ...
    if max_price_usd is not None and not max_price_usd > 0:
        return {
            ...
            "reason": "price ceiling must be positive",
        }
    ...
    try:
        ...
        if store is not None:
            ...
            if matched_store is None:
                return {"ok": False, "error": "not_found", "reason": f"no store '{store}'"}
...
def find_order(ctx: AuthContext, query: str) -> dict[str, Any]:
    ...
    try:
        ...
        for product in db.list_products(conn):
            ...
            if words:
                # Token matching tolerates typos and extra conversational words.
                scores[product.id] = sum(
                    bool(get_close_matches(word, query_words, n=1, cutoff=0.8))
                    for word in words
                ) / len(words)
